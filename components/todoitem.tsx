  ...
  return (
    <Draggable draggableId={todo.id.toString()} index={index} key={todo.id}>
      {(draggableProvided, draggableSnapshot) => (
        <form
          className="flex rounded-md bg-gray-300  w-full p-[20px] mt-[15px] transition hover:scale-105 hover:shadow-md bg-opacity-20 backdrop-blur-xl border-2 border-gray-200 drop-shadow-lg hover:bg-opacity-5 z-auto"
        ...
        >
          {todo.isDone ? (
            ...
            <input
              autoFocus
              className="text-gray-100 w-72 px-1 py-2  outline-none rounded-md bg-gray-800 bg-opacity-40 border-2 border-yellow-400 border-dotted"
              type="text"
              ref={inputRef}
              value={editName}
            ...
            <span className="flex-1">
              {todo.name?.substr(0, 6) +
                `....` +
                todo.name?.substr(todo.name.length - 5, todo.name.length - 20)}
            </span>
          )}
